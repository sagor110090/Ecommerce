                ...
                @foreach ($customer_packages as $key => $customer_package)
                    <div class="col-xl-3 col-lg-4 col-md-6">
                        <div class="dashboard-widget text-center mt-4 c-pointer">
                            ...
                            <hr>
                            @if(!Auth::check())
                                @if ($customer_package->id != '1')
                                    <button class="btn btn-base-1 w-100" onclick="show_price_modal({{ $customer_package->id}})">{{__('Get This')}}</button>
                                @else
                                    <button class="btn btn-base-1 w-100">{{__('Free Package')}}</button>
                                @endif
                            @elseif(Auth::check())
                                @if ($customer_package->id == Auth::user()->customer_package_id)
                                    <button class="btn btn-base-1 w-100" onclick="show_price_modal({{ $customer_package->id}})">{{__('Current Package')}}</button>
                                @else
